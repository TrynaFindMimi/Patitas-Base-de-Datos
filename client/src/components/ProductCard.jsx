...
export default function ProductCard({ product }) {
  ...
  return (
    <div className="bg-white brutal-border brutal-shadow rounded-2xl overflow-hidden hover-lift group relative">
      {stockLabel && (
        <div className={`absolute top-3 left-3 z-10 sticker animate-bounce-in ${stock === 0 ? 'bg-red-600' : 'bg-amber-500'} text-white`}>
          {stockLabel}
        </div>
      )}
      ...
      <div className="p-4">
        ...
        {product.description && (
          <p className="text-xs text-text-muted leading-relaxed line-clamp-2 mb-2">{product.description}</p>
        )}
        ...
        <div className="flex items-center justify-between">
          ...
          >
            {stock === 0 ? 'Agotado' : inCart ? 'Agregado!' : 'Agregar'}
          </button>
        </div>
      </div>
    </div>
